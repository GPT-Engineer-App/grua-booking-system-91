...
import {
  TowingRequestCreateDto,
  TowingRequestUpdateDto,
} from "./towingRequest.dto";
...
export class TowingRequestController {
  ...
  async create(@Body() createDto: TowingRequestCreateDto) {
    const towingRequest = await this.towingRequestDomainFacade.create(
      createDto
    );
    this.eventService.emit(
      new TowingRequestApplicationEvent.TowingRequestCreated(
        towingRequest.id,
        towingRequest.userId
      )
    );
  ...
  async update(
    @Param("id") id: string,
    @Body() updateDto: TowingRequestUpdateDto
  ) {
